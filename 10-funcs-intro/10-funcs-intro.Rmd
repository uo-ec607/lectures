...
FP allows to avoid the explicit use of loop constructs and its associated downsides. In practice, there are two ways to implement FP in R: 1) The `*apply` family of functions and 2) The `purrr` package.

### The `*apply` family (`lapply` and co.)

#### `lapply` basics

Base R contains a very useful family of `*apply` functions. I won't go through all of these here --- see `?apply` or [this blog post](https://nsaunders.wordpress.com/2010/08/20/a-brief-introduction-to-apply-in-r/) among numerous excellent resources --- but they all follow a similar philosophy and syntax. The good news from our perspective is that this syntax very closely mimics the basic for-loop syntax. For example, consider the below code that is analgous to our first for-loop above, but now invokes an `lapply` call instead. `lapply` can take various input types as arguments --- vectors, data frames, lists --- but always returns a list. (Where each element of the returned list is the result from one iteration of the loop.) So now you know where the "l" in "**l**apply" comes from...

```{r lapply_ex}
lapply(1:10, function(i) LETTERS[i])
```

A couple of things to notice. First, check your "Environment"" pane in RStudio. Do you see an object called "i" in the Global Environment? No! Again, this is because the objects and "hyperparameters" created (and invoked by) a function are all evaluated in a sandbox environment outside of your global environment.

and you would have had to pipe the above to `unlist()` if you instead wanted a vector. While this default list return behaviour may not sound ideal at first, I've found that I use `lapply` much more frequently than any of the other `apply` family members. One reason is that it works very well with data frames, especially when combined with `dplyr::bind_rows()`. The latter function allows us to concatenate a list of data frames into a single data frame. For example, here's a a slightly modified version of the function that produces a useful data frame output:

```{r bind_rows_ex, warning=F}
library(tidyverse) ## Already loaded

lapply(1:10, function(i) {
  data.frame(num = i, let = LETTERS[i])
  }) %>%
  bind_rows()
```
...
Probably my favourite thing about the `pblapply` function, though is that it allows for easy implementation of parallel (i.e. multicore) processing that works *across operating systems*. We'll cover this next week.
