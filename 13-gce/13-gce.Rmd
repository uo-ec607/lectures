...
I don't think it's strictly necessary, but I'm going to install the development version of `googleComputeEngineR`. I'm also going to hold off loading it until I've enabled auto-authentication later in the lecture.
...
Thus far in the course, we've spent quite a bit of time learning how to code efficiently. We've covered topics like [functional programming](https://raw.githack.com/uo-ec607/lectures/master/10-funcs-intro/10-funcs-intro.html#functional_programming), [results caching](https://raw.githack.com/uo-ec607/lectures/master/11-funcs-adv/11-funcs-adv.html#caching_(memoization)), [parallel programming](https://raw.githack.com/uo-ec607/lectures/master/12-parallel/12-parallel.html), etc. All of these tools are designed to make the most of the computational resources at your disposal. However, there's a limit to how far they can take you. At some point, datasets become too big, simulations become too complex, and regressions take too damn long to run to run on your local computer. The only solution then is more power.

### To the cloud!

At the risk of gross simplification, there are two ways to "brute force" your way through a computationally-intensive problem.^[Of course, you can also combine a cluster of very power machines to get a *really* powerful system. That's what supercomputing services like the University of Oregon's [Talapas cluster](https://hpcf.uoregon.edu/content/talapas) provide, which is the subject of our next lecture.]
...
Today, we're going to explore cloud-based implementations of these two approaches. Of course, the cloud is not the only game in town and there are various "local" solutions available too (e.g. ultra high-end desktops or local servers). However, the cloud offers offers a variety benefits that make it a no-brainer for most people. It's typically much cheaper due to economies of scale and efficient use of resources. Maintenance and depreciation worries are taken care of. Access does not hinge on institutional affiliation or faculty status (or lack thereof). Cloud providers offer a menu of services that extend far beyond just having access to more powerful computers. And so on. Without exaggeration, knowing how to access and use these cloud resources has made massive difference to my daily workflow and research effectiveness. It has enabled me to complete projects and conduct analysis that would have been virtually impossible otherwise. I expect it could do the same for you.

In particular, we're going to explore various big data products from [**Google Cloud Platform**](https://console.cloud.google.com/) (**GCP**).^[Alternatives to GCP include [AWS](https://aws.amazon.com/) and [Digital Ocean](https://www.digitalocean.com/). RStudio recently launched its own cloud service too: [RStudio Cloud](https://rstudio.cloud/) is more narrowly focused, but is great for teaching and currently free to use. The good news is that these are all great options and the general principles of cloud computing carry over very easily. So use whatever you feel comfortable with.]


### Virtual machines

In particular we're going to use [**Google Compute Engine**](https://cloud.google.com/compute/) (**GCE**) to spin up virtual machines in the cloud that we can then interact with from our local computers. 

At this point you may be asking yourself: "What is a virtual machine and why do I need one anyway?" 

In the simplest sense, a [virtual machine](https://en.wikipedia.org/wiki/Virtual_machine) — or VM — is just an emulation of a computer running inside another (bigger) computer. It can potentially perform all or more of the operations that your physical laptop/desktop does, and it might have many of the same properties (from operating system to internal architecture.) The key advantage of a VM from our perspective is that very powerful machines can be "spun up" in the cloud almost effortlessly and then deployed to tackle jobs that are beyond the capabilities of your local computer. Got a big dataset that requires too much memory to analyse on your old laptop? Load it into a high-powered VM. Got some code that takes an age to run? Fire up a VM and let it chug away without consuming any local resources. Or, better yet, write code that [runs in parallel](https://raw.githack.com/uo-ec607/lectures/master/12-parallel/12-parallel.html) and then spin up a VM with lots of cores to get the analysis done in a fraction of the time. All you need is a working internet connection and a web browser.

### Google Compute Engine

Now, with that background knowledge in mind, [**Google Compute Engine**](https://cloud.google.com/compute/) (**GCE**) delivers high-performance, rapidly scalable VMs in the cloud. A new VM can be deployed or shut down within seconds, while existing VMs can easily be ramped up or down depending on a project's needs (cores added, RAM added, etc.) In my experience, most people would be hard-pressed to spent more than a couple of dollars a month using GCE once their free trial is over. This is especially true for researchers or data scientists who only need to fire up a VM, or VM cluster, occasionally for the most computationally-intensive part of a project, and then can easily switch it off when it is not being used.
...
Okay, introduction out of the way. Let's get up and running. Our goal is to set up a VM (or cluster of VMs) on GCE. What's more, we want to install R and RStudio (Server) on these VMs, so that we can interact with them in exactly the same way that we're used to on our own computer. I'm going to show you two approaches:

1. Manually configure GCE with RStudio Server
2. Automate with googleComputeEngineR and friends

Both approaches have their merits, but I think it's important to start with the manual configuration, so you get a good understanding of what's happening underneath the hood.

## Option 1: Manually configure GCE with RStudio Server

**Note:** It's possible to complete nearly all of the steps in this section via the [GCE browser console](https://console.cloud.google.com/compute/instances). However, we'll stick to using the [shell](https://raw.githack.com/uo-ec607/lectures/master/03-shell/03-shell.html), because it will make it easier to document our steps. It will also save us some headaches further down the road, e.g. when it comes to transferring files between your local computer and a GCE instance *en masse*. 
...
2) Take note of the External IP address for step 3 below.
  ...
  ```
3) Open up a web browser and navigate to RStudio Server on your VM. Enter your username/password as needed. http://EXTERNAL-IP-ADDRESS:8787
...
There's a lot more that you could do with the previous example. For instance, you could add your own Docker images to the `gve_vm()` call so that your entire environment is ready to go when the VM is instantiated (see [here](https://cloudyr.github.io/googleComputeEngineR/articles/docker.html).) For this second example, however, I want to show you how to spin up a simple cluster of VMs that you can interact with *directly* from your local RStudio instance via the [future.apply package](https://github.com/HenrikBengtsson/future.apply). Yes, you read that correctly. 
...
For this cluster example, we're going to use the same `reg_func()` bootstrapping function that we saw in the previous lecture --- just very slightly amended since our VMs will only set up to use base R functions (i.e. no tidyverse).^[I mean, we could easily use the `rocker/tidyverse` Docker image template, but that will take longer to install for this simple example.]
...
And look at that: A three times speedup! 
