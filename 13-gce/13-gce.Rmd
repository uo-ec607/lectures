...
I don't think it's strictly necessary, but I'm going to install the development version of `googleComputeEngineR`. I'm also going to hold off loading it right now, because I first want to set up auto-authentication later in the lecture.
...
reg_func <- 
  ...
  ## Return value
  return(data.frame(x_coef = x_coef))
  }
```
...
## This next chunk function jobs that are creating VMs in background
jobs <- 
  lapply(
    vm_names, 
    function(x) {
...
plan(list( 
  tweak(cluster, workers = as.cluster(vms)),  # Use a cluster of computers locally 
  tweak(multiprocess)  # Use all the CPUs on remote machines 
  )) 

set.seed(123) ## Optional for replication

tic()
future_sim <- future_lapply(1:10000, reg_func) %>% bind_rows()
toc()
```
