...
Many "big data" problems could be accurately described as "small data problems in disguise". Which is to say, the data that we care about is only a subset or aggregation of some larger dataset. For example, we might want to access US Census data... but only for a handful of counties along the border of two contiguous states (for example, Oregon and Washington). Or, we might want to analyse climate data collected from a large number of weather stations... but aggregated up to the national or monthly level. In such cases, the underlying bottleneck is interacting with the original data, which is too big to fit into memory. How do we store data of this magnitude and effectively access it? Enter **relational databases**. 

Databases can exist either locally or remotely, though the latter is more common.^[For the rest of this lecture, I'll be using "databases" and "relational databases" interchangeably, so don't be put off if I switch between them.] The key point is that they are stored on-disk, rather than in-memory. We can then **query** the database to manipulate and/or extract the bits of information that we want from this much larger information set. Once that query is complete, we can pull this more manageable data into memory and continue with our analysis as per usual.

At this point, you might be tempted to think of a database as the "thing" that you interact with directly. However, it's important to realise that the data are actually organised in one or more tables within the database. These tables are rectangular, consisting of rows and columns, where each row is indentified by a unique key. In that sense, they are very much like the tidy data frames that we've been using throughout this course. Continuing with the analogy, a database is rather like a list of data frames of R. To access information from a specific table (data frame), we first have to index it from the database (list) and then execute our query functions. The only material difference being that the database can hold much more information and is extremely efficient at executing queries over its vast contents.


## Databases and the tidyverse

Speaking of queries, virtually relational database in existence makes use of [**SQL**](https://en.wikipedia.org/wiki/SQL) (**S**tructured **Q**uery **L**anguage ).  SQL is an extremely powerful tool and has become something of prerequisite for many data science jobs. ([Exhibit A](https://raw.githack.com/uo-ec607/lectures/master/01-intro/01-Intro.html#21).) However, it is also an archaic language that is much less intuitive to work with than the tidyverse tools that we have been using to manipulate data thus far in the course. We'll see several examples of this shortly, but first the good news: You already have all the programming skills you need to start working with databases. This is because the tidyverse --- through `dplyr` --- allows for direct communication with databases. 

What does this mean? Simply that you can interact with the vast datasets that are stored in relational databases and tables using *exactly* the same tidyverse verbs and syntax that we already know! You don't even need to learn SQL.
...
All of this is possible thanks to the [**dbplyr** package](https://dbplyr.tidyverse.org/), which provides a database backend to `dplyr`. What's happening even further behind the scenes is that, upon installation, `dbplyr` suggests the [**DBI** package](https://db.rstudio.com/dbi) as a dependency. `DBI` provides a common interface that allows `dplyr` to work with many different databases using the same code. 
...
This next section is largely lifted directly from Hadley Wickham's excellent tutorial on [Databases using dplyr](https://db.rstudio.com/dplyr). 
...
Now that you've hopefully absorbed the key principles of database interaction from our demonstration example, let's move on to a more realistic use-case. In particular, the database service that I rely on for my own daily workflow is [**Google BigQuery**](https://cloud.google.com/bigquery/). The advertising tagline is that BigQuery is a "serverless, highly scalable, enterprise data warehouse designed to make all your data analysts productive at an unmatched price-performance." We needn't get sucked into the marketing spin and technical jargon. However, I think it's fair to say that BigQuery is a fantastic product. The salient features from our perspective are: 
...
birth_weight <-
  ...
  summarise(weight_pounds = mean(weight_pounds)) %>% 
  collect()
```
...
```{r birth_weight_st_plot}
birth_weight_st %>% 
  mutate(gender = ifelse(is_male, "Male", "Female")) %>%
  ggplot(aes(year, weight_pounds, col=gender)) + 
  geom_line() + 
  facet_wrap(~state)
```
