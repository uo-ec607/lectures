...
While we've just seen that it's possible to include raw SQL queries in R, a safer and more integrated approach is to use the `glue_sql()` function from the [**glue**](https://glue.tidyverse.org/) package. This will allow you to 1) use local R variables in your SQL queries, and 2) divide long queries into sub-queries. Here's a simple example of the former.
