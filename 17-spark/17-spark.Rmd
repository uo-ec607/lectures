...
log_mod <- 
  flights_train %>%
  ml_logistic_regression(late ~ dep_delay + dep_block)
...
rf_mod <- 
  flights_train %>%
  ml_random_forest(late ~ dep_delay + dep_block, type = "classification")
...
## Preview the prediction results
# rf_pred %>% 
#   select(late, dep_block, prediction, contains("probability")) %>% 
#   head(5)
...
We can compare our two confusion matrices now. The logistic model does (slightly) better on true positives, whilst the random forest does (slightly) better on true negatives. Both tend to underestimate the number of late arrivals.
