...
- New: **sparklyr**, **dbplot**
- Already used: **tidyverse**, **DBI**, **hrbrthemes**, **lubridate**, **janitor**, **httr**, **here**
...
pacman::p_load(tidyverse, DBI, hrbrthemes, lubridate, janitor, httr, 
...
These two commands should run quickly. The required memory usage should also be barely discernible, which again highlights some key advantages of the shell. 
...
**Aside 2:** Something else these examples don't demonstrate --- but I want to emphasise nonetheless --- is that Spark can also read compressed (.gzip and .bz2), as well as parquet file types. This is obviously important for efficient disk useage when dealing with big data.
...
col_names = 
  read.csv(here("17-spark/data/airOT201201.csv"), nrows = 1) %>% 
  clean_names() %>%
  names()
```
...
This next step is not strictly necessary, but since we'll no longer be using the original "air" table anymore, it's safe to remove it and conserve memory for other Spark objects. We can do this with the `DBI::dbRemoveTable()` function.

```{r rm_air, cache=TRUE}
dbRemoveTable(sc, "air")
```

(You may need to refresh your Connections pane to confirm that the "air" table is no longer there.)

Alright, let's query and mutate some columns from our new "flights" table.^[You might note that that I'm not using **lubridate** for the date conversions in the query that follows. That's because Spark relies on Hive functions for built-in aggregation, including datetime operations. See [here](https://spark.rstudio.com/dplyr/#hive-functions).] I'm also going to go ahead and cache the resulting table in Spark's memory allocation to improve performance. There are various configurations and considerations that we'd normally want to weigh before caching a Spark table --- see [here](https://spark.rstudio.com/guides/caching/) --- but the default settings will suffice for this simple example. In truth, the benefit from explicit caching is not clear for this particular case, since the whole dataset is small enough to be held in memory regardless.^[By default, local Spark instances only request 2 GB of memory for the Spark driver. However, you can easily change this by requesting a larger memory allocation (among other features) via the `config` argument. See the **sparklyr** [website](https://spark.rstudio.com/deployment/#configuration) or [Chapter 9](https://therinspark.com/tuning.html) of *Mastering Spark with R* for more details.] Again, I just want to demonstrate some general principles that would apply with more relevancy when working with bigger datasets. 

```{r flights_cached, cache=TRUE}
flights_cached = 
  flights %>% 
  select(
  ...
  mutate(across(-fl_date, as.numeric)) %>%
  ## Register resulting Spark SQL in Spark connection
  sdf_register("flights_cached")

## Cache it (i.e. load into memory for better performance)
...
And there we have our same plot from before, but now executed from inside the Spark table, which in turn was created from a directory of distributed files. Pretty cool and very powerful once you start working with big data proper.
