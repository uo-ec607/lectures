...
- **New:** `sparklyr`
- **Already used:** `tidyverse`, `hrbrthemes`, `lubridate`
...
pacman::p_load(tidyverse, hrbrthemes, lubridate, sparklyr)
...
While the above method is great for showing off the power of the shell and the basic functionality of `sparklyr`, it doesn't demonstrate the core distributed functionality of the Spark ecosystem. So, here's an arguably better approach that does just that. 

Let's start by removing that `combined.csv` file from earlier, just to convince ourselves that we're not somehow reading in the shell-concatenated file. We want to make sure that we're working across a distributed set of files.
...
cached_air_distr <- 
  ...
  compute("cached_air_distr")

cached_air_distr %>% 
  count()

head(cached_air_distr, 5)
...
Finally, let's disconnect from our connection.
```{r discon, cache=F}
spark_disconnect(sc)
```
