...
Remembering that our "flights" table still only exists as pointers to the files on disk, here's something cool: We can use run a query and/or then use the **dbplot** package ([link](https://db.rstudio.com/dbplot/)) to perform plot calculations *inside* the Spark connection (i.e. database). This is essentially like having a **ggplot2** backend for database connections.
...
```{r lg_pred}
lg_pred = ml_predict(lg_mod, flights_sample$test)
...
## Run predictions on test (i.e. holdout) sample
nn_pred = ml_predict(nn_model, flights_sample$test)
