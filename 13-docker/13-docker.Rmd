...
- **Mac/Windows:** Type in `$ docker inspect <containerid>  | grep IPAddress` to get your IP address (see [here](https://stackoverflow.com/a/46310428)). Note that this information was also displayed when you first launched your Docker Quickstart Terminal. For example:
...
```{bash}
docker run `-d` -p 8787:8787 -e PASSWORD=pswd123 rocker/tidyverse
```
