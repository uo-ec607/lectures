...
Again, this should feel _very_ familiar to our Git workflow. We even wrote ourselves a helpful commit message. Note that I added a "tag" to the image (i.e. `4.0.0`). This is optional, but good practice. Here, I'm reminding myself that I've built on top of the R 4.0.0 versioned stack.
...
For our simple example, I installed a single R package. But the process would work exactly the same if I did any other operations from within R(studio).
...
**Question:** What happens if I want to install/update something outside of R(Studio)? Say, Ubuntu system libraries or another program like Python?
