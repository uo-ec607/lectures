...
We'll also be working with our simple `square()` function from the previous lecture. Let's create it again quickly.
...
Fortunately, R has our back with several tools. I'm going to focus here on the [R.cache package]
(https://cran.r-project.org/web/packages/R.cache/index.html) from [Henrik Bengtsson](https://twitter.com/henrikbengtsson). I should perhaps also clarify that there are various forms of caching, but we're going to be focused on what's known as [memoization](https://en.wikipedia.org/wiki/Memoization).
...
- Amanda Gadrow has an excellent video about [*Debugging techniques in RStudio*](https://www.rstudio.com/resources/videos/debugging-techniques-in-rstudio/) (23 min).
