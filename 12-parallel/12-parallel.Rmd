...
So that took about `r round(as.numeric(gsub(" sec elapsed", "", (tic.log()[[1]]))), 0)` seconds on my system. Not a huge pain, but let's see if we can do better by switching to a parallel (multicore) implementation. For the record, though here is a screenshot of my system monitor, showing that only one core was being used during this serial version.
...
| ×	Only available for Unix-based systems (not Windows). | ✓ Works on every operating system (including Windows). |
