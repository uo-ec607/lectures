...
All of the parallel programming that we've been doing so far is built on top of [Henrik Bengtsson's](https://twitter.com/henrikbengtsson) [**future package**](https://cran.r-project.org/web/packages/future/index.html). As Henrik [describes](https://cran.r-project.org/web/packages/future/vignettes/future-1-overview.html) it:
...
As I've tried to emphasise, `future` is a relatively new package, and is certainly not the first or only way to implement parallel processes in R. However, I think that `future` provides a simple and unified framework that makes it the preeminent choice. What's more, the same commands that we use here will carry over very neatly to more complicated settings involving high-performance computing clusters. (We'll experience this first hand when we get to the big data section of the course.) 
...
One other parallel option that I want to mention very briefly is the [pbapply package](https://github.com/psolymos/pbapply). As we saw during the first programming lecture, this package provides a lightweight wrapper on the *apply functions that adds a progress bar. However, the package also adds a very convenient option for multicore implementation. You basically just have to add <code>cl=CORES</code> to the call. See [here](http://peter.solymos.org/code/2016/09/11/what-is-the-cost-of-a-progress-bar-in-r.html) for an interesting discussion on what's happening behind the scenes.
...
As I keep saying, it's now incredibly easy to run parallel programs in R. The truth is that it has actually been easy to do so for a long time... but the implementation used to vary by operating system (e.g. Linux/Mac vs Windows). For example, take a look at the [help documentation](https://stat.ethz.ch/R-manual/R-devel/library/parallel/html/mclapply.html) for the `parallel::mclapply()` function, which has been around since 2011. If you did so, you would see a warning that `mclapply()` *"relies on forking and hence is not available on Windows"*.

Now, we obviously didn't encounter any OS-specific problems when we ran the parallel versions of our motivating example above. That includes anyone using Windows. ~~*Loud booing.*~~ What was happening behind the scenes is that the parallel functions were being executed in a way that was automatically optimised for your respective OS. This was all done via the new [future package](https://cran.r-project.org/web/packages/future/index.html) ecosystem, which we'll turn to shortly.
...
Since this is only in effect for the current R session, BLAS multithreading will be restored when I restart R.^[I could also reinstate the original behaviour in the same session by running `blas_set_num_threads(parallel::detectCores())`. You can turn off multithreading as the default mode by altering the configuration file when you first build/install your preferred BLAS library. However, that's both complicated and unecessarily restrictive in my view.] Interestingly, turning off BLAS multi-threading has only a modest impact on performance. The reason is that most of the benefit provided by optimised BLAS libraries actually comes from improved math vectorisation, not multi-threading. See [this blog post](https://blog.revolutionanalytics.com/2015/10/edge-cases-in-using-the-intel-mkl-and-parallel-programming.html) for a detailed discussion by Andrie de Vries of Revolution Analytics.

## Miscellaneous

### When should I go parallel?

The short answer is that you want to invoke the multicore option whenever you are faced with a so-called "[embarrassingly parallel](https://en.wikipedia.org/wiki/Embarrassingly_parallel)" problem. You can click on that link for a longer description, but the key idea is that these computational problems are embarrassingly easy to break up into smaller chunks. You likely have such a case if the potential code chunks are independent and do not need to communicate in any way. Classic examples include bootstrapping of standard errors (since each regression or resampling iteration is drawn independently) and Markov chain Monte Carlo (i.e. [MCMC](https://en.wikipedia.org/wiki/Markov_chain_Monte_Carlo)).
...
### Fault tolerance (error catching and caching)
...
A number of regression packages in R are optimised to run in parallel. For example, the superb [lfe package]https://cran.r-project.org/web/packages/lfe/index.html) that we saw a few weeks ago in the lecture on regression analysis^[See [here](https://raw.githack.com/uo-ec607/lectures/master/08-regression/08-regression.html#fixed_effects_with_the_lfe_package).] will automatically invoke multicore capabilities when fitting high dimensional fixed effects models. The many Bayesian packages in R are also all capable of --- and, indeed, expected to --- fit regression models by running their MCMC chains in parallel (e.g. [Stan](https://cran.r-project.org/web/packages/rstan/vignettes/rstan.html#running-multiple-chains-in-parallel)). Finally, you may be interested in the [partools package](https://cran.r-project.org/web/packages/partools/index.html), which provides convenient aliases for running a variety of statistical models and algorithms in parallel.
