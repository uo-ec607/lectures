...
Fixed effects models are more common than random effects models in economics (in my experience, anyway). I'd also advocate for [Bayesian hierachical models](http://www.stat.columbia.edu/~gelman/arm/) if we're going down the whole random effects path. However, it's still good to know that R has you covered for random effects models through the [plm package](https://cran.r-project.org/web/packages/plm/).^[As I mentioned above, `plm` also handles fixed effects models (as well as pooling models). However, I prefer `lfe` for the reasons already discussed.]

```{r ols6a_re, error=T}
ols6_re <- 
  plm(
    mass ~ height, 
    data = starwars %>% filter(!is.na(species)), 
    index=c("species"), 
    model="random"
    )
summary(ols6_re)
...
```{r bayes_reg, error=T, warning=F, results="hide"}
