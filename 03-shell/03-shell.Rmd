...
You couldn't tell here, but it's worth noting that the character count is actually higher than we'd get if we (bothered) counted by hand.
- Reason: It counts the invisible newline character "/n".
...
Again, using Shakespeare's Sonnets.
...
- Using `>` will just try to rewrite the existing file contents.
...
- In other words, it allows us to chain together a sequence of simple operations and thereby implement a more complex operation. (DOTADITW!)
...
Now let's say that I want to pull out all of text from (but limited to) Sonnet 18. 
...
# Sorting and removing repetition: sort

```{bash reps}
cat examples/reps.txt
```

There is quite a bit of repetition here (and a double entendre). Let's go about fixing that. 

We can remove duplicate lines in various ways in Bash, but here I'll go for a simple `sort` command using the `-u` ("unique") flag. 
- I'll also add a `-r` ("reverse") flag, but only because `sort` orders alphabetically and this makes less sense for this simple example.
...
- For loops, variables and functions, scripts, SSH, memory control (e.g. top and htop), etc.
