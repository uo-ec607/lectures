...
☑ Do you have Bash-compatible shell? (Windows users: see [here](#windows).)
...
- Windows users need to install a Bash-compatible shell (see [here](#windows)).
...
_for_ loops in Bash work similarly to other programming languages that you are probably familiar. 
...
- First, create the new file by grapping the header (i.e. top line) from any of the input files and redirecting it. No need for `touch` this time.
...
I'm going to go out on a limb and recommend option 2 (WSL) if available to you. It's more overhead, but I think worth it. See the next slide for some extra instructions...
...
- Follow the guide to install your preferred Linux distro first. Ubuntu is a good choice. 
- Then, come back to these slides.

After installing the WSL, you need to navigate to today's lecture directory to run the examples. 
- Presumably, you've cloned the course repo somewhere on your (Windows) C drive. 
