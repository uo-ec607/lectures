...
All shell commands have the same basic syntax:
...
I also like the `cheat` utility, which provides a readable summary / cheatsheet for a command. You'll need to [install](https://github.com/chrisallenlane/cheat) it first. (Linux and MacOS only.)
```bash
$ cheat ls
```
...
**Answer:** Bash syntax is unapologetically strict about spaces and ordering. Here it thinks that "My" and "Documents" are separate arguments.
...
- Navigate to the "exercises" subfolder of this lecture folder and list the files with the `-lh` option ("long format", "human readable").

```bash
$ cd examples ## Assumes you are starting off in the "03-shell" lecture folder
$ ls -lh
total 136K
drwxr-xr-x 2 grant users 4.0K Jan 12 22:02  ABC
drwxr-xr-x 2 grant users 4.0K Jan 12 14:53 'Bad folder name'
drwxr-xr-x 2 grant users 4.0K Jan 12 21:33  meals
-rwxr-xr-x 1 grant users   48 Jan 12 22:13  hello.sh
-rw-r--r-- 1 grant users  153 Jan 12 21:20  reps.txt
...
<span style='background: #ffff88;'>d</span>rwxr-xr-x 2 grant users 4.0K Jan 12 22:02 ABC</span>
...
<p span style="font-family:Fira Code; font-size:80%; color: #333; background: #f8f8f8; padding: 0.5em;";>d<span style='background: #ffff88;'><span style='color: #e41a1c;'>rwx</span><span style='color: #377eb8;'>r-x</span><span style='color: #4daf4a;'>r-x</span></span> 2 grant users 4.0K Jan 12 22:02 ABC</span>
...
<p span style="font-family:Fira Code; font-size:80%; color: #333; background: #f8f8f8; padding: 0.5em;";>drwxr-xr-x <span style='background: #ffff88;'>2</span> grant users 4.0K Jan 12 22:02 ABC</span>
...
<p span style="font-family:Fira Code; font-size:80%; color: #333; background: #f8f8f8; padding: 0.5em;";>drwxr-xr-x 2 <span style='background: #ffff88;'><span style='color: #e41a1c;'>grant</span> <span style='color: #377eb8;'>users</span></span> 4.0K Jan 12 22:02 ABC</span>
...
<p span style="font-family:Fira Code; font-size:80%; color: #333; background: #f8f8f8; padding: 0.5em;";>drwxr-xr-x 2 grant users <span style='background: #ffff88;'>4.0K Jan 12 22:02 ABC</span></span>
...
*Disclaimer: This next section is tailored towards Unix-based operating systems (Linux or MacOS). However, it will become relevant to everyone (even Windows users) once we start interacting with Linux servers, spinning up Docker containers, etc. later in the course.*
...
To fully switch over to the superuser, you can type `sudo -s`. 
...
It is generally considered very poor practice to switch over to root for long periods, since you needlessly risk messing up your system. 
- Remember there's no "undo" option when working through the shell.
...
*Note: You have to be a "permitted" user to invoke `sudo`. This should be enabled by default on your personal laptop, but if not see [here](https://www.thegeekstuff.com/2010/09/sudo-command-examples/embed/) or [here](https://www.digitalocean.com/community/tutorials/how-to-create-a-sudo-user-on-ubuntu-quickstart).*

---

# Changing permissions and ownership

Let's go back to the folder that we saw previously.
...
Note that changing permissions via the Symbolic method doesn't need the whole entry. E.g. To go from the first line to the second, I'd only need `$ chmod o-rwx myfile`
