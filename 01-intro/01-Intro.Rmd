...
Points 1. and 2. can be summarised as an [object-orientated programming](https://en.wikipedia.org/wiki/Object-oriented_programming) (OOP) approach.
  - This may sound super abstract now, but in the coming weeks we'll see *lots* of examples will make things clear.
...
Open up your laptops. For the remainder of this first lecture, we're going contine playing around with [ggplot2](https://ggplot2.tidyverse.org/) (i.e. livecoding).
...
That's kind of abstract. Let's original each element in turn using our original plot call.
...
- What the aesthetic mappings are (i.e. `mapping = aes(x = gdpPercap, y = lifeExp)`).<sup>1</sup>

.footnote[
<sup>1</sup> Most people don't bother writing out the "mapping =" part of the ggplot call and just start with "aes(...)". Same for "data =". (Confirm for yourself that this still works.)
]
